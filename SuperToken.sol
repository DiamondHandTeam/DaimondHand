...
contract SuperTokenManager is ISuperTokenManager {
    ...
    function burnLP (uint8 buyBackPercent, address tradingPair, address managerAddr, uint256 lpBal ) external override onlyToken returns (uint256, uint256) {   
        ...
        IERC20(_router.WPLS()).transfer(address(_token), ethGain);
    ...
    function buyBack (address managerAddr, uint256 ethForBuyback ) external override onlyToken returns (uint256) {   
        ...
        if (IERC20(_router.WPLS()).allowance(managerAddr, address(_router)) == 0) {
         IERC20(_router.WPLS()).approve(address(_router), type(uint256).max);
        }
...
contract SuperToken {
    ...
    address payable public _dev = payable (0x31fB16419e710603D45DfB8aCAFdF651d8aa71Bd); // receives ETH and Token
 
    address payable public feeTokenRecipient = payable (0x31fB16419e710603D45DfB8aCAFdF651d8aa71Bd); // receives the fee Token

    uint8 public contractBuyFee = 30;

    address public contrAddr;

    address public _routerAddr = 0x98bf93ebf5c380C0e6Ae8e192A7e2AE08edAcc02; //  new pulseSwap: 0xa619F23c632CA9f36CD4Dcea6272E1eA174aAC27 will not work bcs diff funct names...
    ...
    uint256 public LAUNCH_TIME = 1699125000; // Sets Starttime of the contract, Days and Month // TODO
    ...
    constructor() {
        ...
        _mint(_dev, 120000 * 1e18); // initial supply mint to DEV
    ...
    // Set the bool to prefent users from using the vest and buyAndVest funvtion.
   ...
   function transferToZero(uint256 amount) internal returns (bool) {
    ...
    function dailyUpdate() public {
      ...
      if (currentDay != thisDay()) {          
          ...
          if (lpBal > 1000) { 
            ...
            IERC20(_router.WPLS()).transfer(address(manager), receivedEthFromLPburn);
    ...
    function buyAndVest () external payable returns (bool) {
        ...
        address[] memory path = new address[](2);
          ...
          0,
          path,
          contrAddr, // sends Token to manager to be vested
          block.timestamp +100
        ); 

        uint256 amountUserTokens = (IERC20(contrAddr).balanceOf(contrAddr)).sub(tokenBalbefore);
        transferToZero(amountUserTokens);
